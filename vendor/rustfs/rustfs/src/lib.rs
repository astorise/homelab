...
async fn async_main() -> Result<()> {
    ...
    bootstrap(config, None).await
}

pub async fn run_embedded(config: Config, shutdown: CancellationToken) -> Result<()> {
    bootstrap(config, Some(shutdown)).await
}

async fn bootstrap(config: Config, shutdown: Option<CancellationToken>) -> Result<()> {
    let config = config;
...
async fn run(config: config::Config, shutdown: Option<CancellationToken>) -> Result<()> {
    debug!("config: {:?}", &config);
