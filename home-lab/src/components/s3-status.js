import { s3_get_status } from '../tauri.js';
import { showError } from './toast.js';

class S3Status extends HTMLElement {
  constructor() {
    super();
  ...
  renderLoading() {
    ...
    this.innerHTML = `
      <div class="p-4 bg-gray-100 rounded">
        <h2 class="font-bold mb-2">S3 Status</h2>
  ...
  renderSuccess(status) {
    this.innerHTML = `
      <div class="p-4 bg-gray-100 rounded">
        ...
        <p>Endpoint: <code>${status.endpoint}</code></p>
        <p>Region: <code>${status.region}</code></p>
        <p>Access key: <code>${status.access_key_id}</code></p>
        <p>Path style: ${status.force_path_style ? 'enabled' : 'disabled'}</p>
      </div>`;
  }
  ...
  async load() {
    ...
    try {
      const status = await s3_get_status();
...
customElements.define('s3-status', S3Status);
