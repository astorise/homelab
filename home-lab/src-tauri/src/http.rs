        ...
        match RpcClient::connect(RPC_INTERFACE_UUID, RPC_INTERFACE_VERSION, endpoint) {
            Ok(client) => {
                info!(
                    ...
                    payload_size,
                    endpoint = endpoint,
                    "Connected to HTTP RPC endpoint"
                );
                trace!(
                    proc = procedure_name(proc_num),
                    call_id,
                    ...
                    Err(err) => {
                        ...
                        if is_transient_rpc_status(code) {
                            warn!(
                                ...
                                code,
                                error = %formatted,
                                "HTTP RPC call transport failed, trying next endpoint"
                            );
                            ...
                            error!(
                                ...
                                code,
                                error = %formatted,
                                "HTTP RPC call failed without recovery"
                            );
                            log_http_diagnostics(&formatted);
